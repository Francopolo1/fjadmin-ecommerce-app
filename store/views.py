...
def search(request):
    ...
    if request.method == "POST":
        ...
        # Test for Null
        if not searched:
            messages.success(request, "That product does not exist... Please try aggain!!")
...
def update_info(request):
    if request.user.is_authenticated:
        ...
        shipping__user = ShippingAddress.objects.get(id=request.user.id)

        # Get original User Form
        form = UserInfoForm(request.POST or None, instance = current_user)
        # Get User's Shipping Form
        shipping_form = ShippingForm(request.POST or None, instance = shipping__user)
...
def login_user(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # Get user Profile
            current_user = Profile.objects.get(user__id=request.user.id)
            # Get user saved cart from database
...
def register_user(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.success(request, ("User name Created - Please fill out user info below..."))
